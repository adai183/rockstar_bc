...
def random_crop(image, steering=0.0, tx_lower=-20, tx_upper=20, ty_lower=-2, ty_upper=2, rand=True):
    ...
    # also the input to the network will need to be cropped, but of course not
    ...
    #    print('tx = ',tx,'ty = ',ty)
    random_crop = image[horizon + ty:bonnet +
                        ty, col_start + tx:col_end + tx, :]
...
#    print('left_center_right  :',lcr)
    image, steering = read_next_image(
        m, lcr, X_train, X_left, X_right, Y_train)
...
#    plt.figure()
#    plt.imshow(image)
    image, steering = random_crop(
        image, steering, tx_lower=-20, tx_upper=20, ty_lower=-10, ty_upper=10)
#    print('steering :',steering)
#    plt.figure()
#    plt.imshow(image)
...
def get_validation_set(X_val, Y_val):
    ...
    for i in range(len(X_val)):
        ...
        X[i], Y[i] = random_crop(
            x, y, tx_lower=0, tx_upper=0, ty_lower=0, ty_upper=0)
    return X, Y
...
batch_size = 200
train_generator = generate_train_batch(
    X_train, X_left, X_right, Y_train, batch_size)
